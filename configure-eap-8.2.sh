...
unzip -q $EAP_ZIP
mv $EAP_ZIP_ROOT jboss-eap-8.2-1

rm -rdf jboss-eap-8.2-2
unzip -q $EAP_ZIP
mv $EAP_ZIP_ROOT jboss-eap-8.2-2
...
echo -e "\n=============================\nConfigure WF 1\n=============================\n\n"
$PWD/jboss-eap-8.2-1/bin/jboss-cli.sh --file=$PWD/wildfly1.cli --echo-command

echo -e "\n=============================\nConfigure WF 2\n=============================\n\n"
$PWD/jboss-eap-8.2-2/bin/jboss-cli.sh --file=$PWD/wildfly2.cli --echo-command

$PWD/jboss-eap-8.2-1/bin/add-user.sh -u admin -p pass.1234
$PWD/jboss-eap-8.2-2/bin/add-user.sh -u admin -p pass.1234

# Build and deploy form-auth-webapp
cd $PWD/form-auth-webapp && mvn -q clean package && cd ..
cp $PWD/form-auth-webapp/target/form-auth-webapp.war $PWD/jboss-eap-8.2-1/standalone/deployments/
cp $PWD/form-auth-webapp/target/form-auth-webapp.war $PWD/jboss-eap-8.2-2/standalone/deployments/

# Start wildfly

echo "$PWD/jboss-eap-8.2-1/bin/standalone.sh --server-config=standalone-ha.xml -Djboss.default.multicast.address=230.0.0.190 -Djboss.node.name=wildfly1 -Djboss.socket.binding.port-offset=100"

echo "$PWD/jboss-eap-8.2-2/bin/standalone.sh --server-config=standalone-ha.xml -Djboss.default.multicast.address=230.0.0.190 -Djboss.node.name=wildfly2 -Djboss.socket.binding.port-offset=400"
